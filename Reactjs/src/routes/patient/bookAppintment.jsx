...
const BookAppointment = () => {
    ...
    useEffect(() => {
        const fetchDoctors = async () => {
            try {
                const response = await axios.get('http://localhost:8080/getDoctors');
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            const formattedTime = moment(values.appointmentTime, 'HH:mm').format('hh:mm A');
            ...
            const response = await api.post('/bookAppointment', updatedValues);
            const decryption = await decryptionProcess(response);
