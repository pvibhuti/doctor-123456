...
exports.uploadDocuments = async (req, res, next) => {
    try {
        ...
        const existingUser = await doctor.findOne({ email: formattedEmail });
        if (!existingUser) {
        ...
        if (existingUser.status === 1) {
            return sendError(req, res, {
                message: "Account Deleted please entre active account.",
            });
        }

        const filePaths = req.files.map(file => file.filename);
        const updateImage = await doctor.findByIdAndUpdate(existingUser._id, { degreeCertificate: filePaths }, { new: true })
...
exports.updateProfilePhoto = async (req, res, next) => {
    try {
        ...
        try {
            ...
            return sendError(req, res, { message: err.name === "TokenExpiredError" ? "Unauthorized! Access denied." : "Invalid token." }, 401);
        }
